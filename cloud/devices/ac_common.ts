...
export default abstract class ACDevice extends TLVDevice {
    ...
    resDeadline: { start?: number; stop?: number } = {}
    ...
    start() {
        ...
        this.powerChangeHooks.push(() => {
            const satisfied = this.powerStatePrev ? 'start' : 'stop'
    ...
    hasStartStopTimers() {
        /* With the relay on the timers exist regardless of what the capability bitmap claims —
         * this driver is the one running them. */
        return this.relayReservation || capBit(this.timerCaps(), CAP_START_STOP_TIMERS)
    }
    ...
    addClimateCore(config: ClimateConfig) {
        ...
        this.addField(config, {
            ...
            read_callback: (val) => {
                ...
                const powerChanged = this.powerStatePrev != null && this.powerStatePrev !== powerState
                this.powerStatePrev = powerState
                if (powerChanged) for (const hook of this.powerChangeHooks) hook()
